...
for i = 1:1:numel(element)
    ...
    switch element(i).type
        ...
        case 2
            ...
            if ~isfield(element(i), 'alpha') || isempty(element(i).alpha)
                element(i).alpha = 1.0 ; % linear stiffness component
            end 
            if ~isfield(element(i), 'beta') || isempty(element(i).beta)
                element(i).beta = element(i).alpha/10; % nonlinear stiffness component
            end
            if ~isfield(element(i), 'gamma') || isempty(element(i).gamma)
                element(i).gamma = element(i).beta/2; % nonlinear stiffness component
            end
            if ~isfield(element(i), 'n') || isempty(element(i).n)
        ...
        case 3
            element(i).nvars = 1;
            if ~isfield(element(i), 'd0') || isempty(element(i).d0)
                element(i).d0 = 1.5e-3;
            end
            if ~isfield(element(i), 'd1') || isempty(element(i).d1)
                element(i).d1 = 0.07;
            end
    ...
    if ~isfield(element(i), 'a0') || isempty(element(i).a0)
        element(i).a0 = 0;
    end
    if ~isfield(element(i), 'a1') || isempty(element(i).a1)
        element(i).a1 = 0;
    end
